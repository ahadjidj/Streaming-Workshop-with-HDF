...
#   curl -sSL ambari-bootstrap.sh | sudo -E sh
#
...
hdf_version="${hdf_version:-3.2.0.0}"
hdf_version_major="${hdf_version_major:-$(echo ${hdf_version} | cut -c 1).x}"
mpack_repo_baseurl="${mpack_repo_baseurl:-http://public-repo-1.hortonworks.com/HDF}"
mpack_tag="${mpack_tag:-520}"
...
case "${lsb_dist}" in
    ...
    case "${lsb_dist_release}" in
        ...
        if [ "${install_ambari_server}" = true ]; then
            ...
            if [ "${install_hdf_mpack}" = true ]; then
                printf "## installing HDF MPack\n"
                mpack_repo="${mpack_repo:-${mpack_repo_baseurl}/${lsb_dist}${lsb_dist_release}/${hdf_version_major}/updates/${hdf_version}/tars/hdf_ambari_mp/hdf-ambari-mpack-${hdf_version}-${mpack_tag}.tar.gz}"
                printf "## MPack URL is ${mpack_repo}\n"            
                sudo cp -r /var/lib/ambari-server/resources /var/lib/ambari-server/resources.backup
                sudo ambari-server install-mpack --verbose --mpack=${mpack_repo}
